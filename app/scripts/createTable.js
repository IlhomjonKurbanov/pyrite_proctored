...
if (tableName == 'SpontaneousResponses' || tableName == 'all') {
    db.get().query('CREATE TABLE SpontaneousResponses(SRID INT NOT NULL AUTO_INCREMENT, SubjectID INT NOT NULL, Trial INT, ArticleID INT, ElementID VARCHAR(255), ThumbsUp BIT(1), PRIMARY KEY (SRID)) ENGINE = INNODB',
        function(err, result) {
            if (err) {
